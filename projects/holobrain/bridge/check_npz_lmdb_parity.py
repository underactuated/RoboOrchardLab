...
from bridge.dataset import BridgeHolobrainDataset
...
def _build_raw_bridge_dataset(npz_root: str, max_episodes: int | None) -> BridgeHolobrainDataset:
    backend = NpzBridgeBackend(root=npz_root, max_episodes=max_episodes)
    return BridgeHolobrainDataset(
        backend=backend,
        constant_depth=train_config.get("bridge_constant_depth", 0.6),
    )
...
def _check_index(
    index: int,
    bridge_raw_dataset: BridgeHolobrainDataset,
    lmdb_raw_dataset: RoboTwinLmdbDataset,
    transforms: list[Any],
) -> dict[str, Any]:
    bridge_raw = bridge_raw_dataset[index]
    lmdb_raw = lmdb_raw_dataset[index]

    raw_results = []
    for key in RAW_KEYS:
        raw_results.append(_compare_value(key, bridge_raw[key], lmdb_raw[key]))

    lmdb_ee_state = _to_numpy(lmdb_raw.get("ee_state"))
    if isinstance(lmdb_ee_state, np.ndarray):
        raw_results.append(
            CompareResult(
                "ee_state",
                lmdb_ee_state.shape[1:] == (0,),
                f"shape={lmdb_ee_state.shape}",
            )
        )

    bridge_transformed = _apply_transforms(bridge_raw, transforms)
    lmdb_transformed = _apply_transforms(lmdb_raw, transforms)

    transformed_results = []
    for key in TRANSFORMED_KEYS:
        transformed_results.append(
            _compare_value(key, bridge_transformed[key], lmdb_transformed[key])
        )
...
def main() -> None:
    ...
    bridge_raw_dataset = _build_raw_bridge_dataset(args.npz_root, args.max_episodes)
    lmdb_raw_dataset = _build_raw_lmdb_dataset(args.lmdb_root, args.max_episodes)

    if len(bridge_raw_dataset) != len(lmdb_raw_dataset):
        raise SystemExit(
            f"dataset length mismatch: npz={len(bridge_raw_dataset)} "
            f"lmdb={len(lmdb_raw_dataset)}"
        )

    transforms = _build_transform_pipeline(args.mode)
    indices = _select_indices(len(bridge_raw_dataset), args.indices)
    results = [
        _check_index(
            index=index,
            bridge_raw_dataset=bridge_raw_dataset,
            lmdb_raw_dataset=lmdb_raw_dataset,
    ...
    output = {
        ...
        "dataset_length": len(bridge_raw_dataset),
